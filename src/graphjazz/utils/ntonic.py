...
def all_ntonic():
    ...
    for Q in (range(1, 2**12)):
        q = Q2q(Q)
        Q_circular = [q2Q(cshift(q, x))
                      for x in range(len(q))]  # list of circular of q
        sw = True
        for i in Q_ntonic:
...
#    q=np.array([0,0,1,0,0,0,1,0,0,0,1,1])
    ...
    for j in [4, 5, 6]:
        ...
        plt.hist(sindex, bins=20, color=cl[j],
                 alpha=0.3, label=str(j)+"-tonic")

    plt.text(3.0, 3, "dim", rotation=90,
             horizontalalignment="center", color="C0")
    plt.text(2.82, 3, "m7, 6", rotation=90,
             horizontalalignment="center", color="C0")
    plt.text(2.1818, 3, "M7", rotation=90,
             horizontalalignment="center", color="C0")

    plt.text(2.307, 3, "major and minor pentatonics", rotation=90,
             horizontalalignment="center", color="C1")
    plt.text(1.5, 3, "(traditional) hexatonic", rotation=90,
             horizontalalignment="center", color="C2")
    plt.text(2.0, 3, "whole tone", rotation=90,
             horizontalalignment="center", color="C2")
